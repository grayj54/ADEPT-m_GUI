function hBuildMenu = openBuildMenu(devObj, newCheck)
% opens build menu using class' current properties
% If newCheck == false, user is editing a preexisting device and object
% parameters are not empty/ don't need to set defaults
% If newCheck == true, user is creating a new device and therefore the
...
% Build Menu Callbacks ----------------------------------------------------
    ...
    function DevBulkPress(hObject, callbackdata)
    ...
    % Build Menu Utility Functions ----------------------------------------
    function val = getTypeNumber
        devType = devObj.type;
        switch (devType)
            case 'Solar Cell'
                val = 1;
