classdef Silicon 
    ...
    methods
         function dev = Silicon()
             dev.ip = a_init_custom;
            
             dev.ltitle = '';
             dev.thick = 0;
             
             dev.ip(3).set = 4.2; %EleAFfin
             dev.ip(4).set = 1.12; %BandGap
             dev.ip(5).set = 11.7; %Dielectric Constant
             dev.ip(6).set = 3.2e19; % Conduction band effective density of states
             dev.ip(7).set = 1.8e19; % Valence band effective density of states (#/cm^3)
             dev.ip(19).set = 1.1e-14;% radiative recombination coeff
             dev.ip(20).set = 1.1e-30; % Auger recombination coeff for electrons
             dev.ip(21).set = 3e-31; % Auger recombination coeff for holes
             dev.ip(10).set = 2.3e7; % electron thermal velocity (cm/s)
             dev.ip(11).set = 1.65e7; % hole thermal velocity (cm/s)
             dev.ip(122).set = 'C-T'; % hole mobility model
             dev.ip(123).set = 461;
             dev.ip(124).set = 54.3;
             dev.ip(125).set = 2.35e17;
             dev.ip(126).set = 0.88;
             dev.ip(117).set = 'C-T'; % electron mobility model
             dev.ip(118).set = 1340;
             dev.ip(119).set = 88;
             dev.ip(120).set = 1.26e17;
             dev.ip(121).set = 0.88;
             dev.ip(22).set = 'file';% for creation of e-h pairs
             dev.ip(116).set = 'si.a';
             dev.ip(13).set = 2e19; % fixed fully ionized dopant (#/cm^3)
             dev.ip(101).set = 2e-10;
             dev.ip(12).set = 1e16; % fixed fully ionized dopant (#/cm^3)
